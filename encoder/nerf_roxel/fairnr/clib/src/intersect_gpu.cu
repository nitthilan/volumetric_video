...
__device__ float2 RayGAABBIntersection(
  const float3 &ori,
  const float3 &dir,
  const float3 &center,
  const float3 &half_voxel) {

  float f_low = 0;
  float f_high = 100000.;
  float f_dim_low, f_dim_high, temp, inv_ray_dir, start, aabb, hv;

  for (int d = 0; d < 3; ++d) {  
    switch (d) {
      case 0:
        inv_ray_dir = __fdividef(1.0f, dir.x); start = ori.x; aabb = center.x; 
        hv = half_voxel.x;
        break;
      case 1:
        inv_ray_dir = __fdividef(1.0f, dir.y); start = ori.y; aabb = center.y;
        hv = half_voxel.y;
        break;
      case 2:
        inv_ray_dir = __fdividef(1.0f, dir.z); start = ori.z; aabb = center.z; 
        hv = half_voxel.z;
        break;
    }
  
    f_dim_low  = (aabb - hv - start) * inv_ray_dir;
    f_dim_high = (aabb + hv - start) * inv_ray_dir;
...
__global__ void gaabb_intersect_point_kernel(
            int b, int n, int m, 
            const float *__restrict__ voxelsize,
            int n_max,
            const float *__restrict__ ray_start,
            const float *__restrict__ ray_dir,
            const float *__restrict__ points,
  ...
  for (int j = index; j < m; j += stride) {
    ...
    for (int k = 0, cnt = 0; k < n && cnt < n_max; ++k) {
      float2 depths = RayGAABBIntersection(
        make_float3(ray_start[j * 3 + 0], ray_start[j * 3 + 1], ray_start[j * 3 + 2]),
        make_float3(ray_dir[j * 3 + 0], ray_dir[j * 3 + 1], ray_dir[j * 3 + 2]),
        make_float3(points[k * 3 + 0], points[k * 3 + 1], points[k * 3 + 2]),
        make_float3(voxelsize[k * 3 + 0]*0.5, 
          voxelsize[k * 3 + 1]*0.5, voxelsize[k * 3 + 2]*0.5));
...
void gaabb_intersect_point_kernel_wrapper(
  int b, int n, int m, const float *voxelsize, int n_max,
  const float *ray_start, const float *ray_dir, const float *points,
  int *idx, float *min_depth, float *max_depth) {
  
  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  gaabb_intersect_point_kernel<<<b, opt_n_threads(m), 0, stream>>>(
      b, n, m, voxelsize, n_max, ray_start, ray_dir, points, idx, min_depth, max_depth);
