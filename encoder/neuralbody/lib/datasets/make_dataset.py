...
def make_feat_data_loader(cfg, is_distributed=False, max_iter=-1):

    args = cfg.train_dataset
    dataset = imp.load_source(cfg.feat_gen.dataset_module, 
        cfg.feat_gen.dataset_path).Dataset(**args)

    cam_bbox = dataset.get_cam_bbox()
    ...
    sampler = make_data_sampler(dataset, shuffle=shuffle, is_distributed=is_distributed, 
        is_train=is_train)
    batch_sampler = make_batch_data_sampler(cfg, sampler, batch_size,
                                            drop_last, max_iter, is_train)
    collator = make_collator(cfg, is_train)
