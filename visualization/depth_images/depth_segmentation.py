...
def read_data(flags):
    ...
    for line in odometry:
        ...
        world_pos.append(C2W)
        poses.append(T_WC)

    world_pos = np.array(world_pos)
    poses = np.array(poses)
    # print("Min Max ", np.min(world_pos, axis=0), np.max(world_pos, axis=0))
    print("Min Max ", np.min(poses, axis=0), np.max(poses, axis=0), poses.shape)
    # print("Intrinsics ", intrinsics, np.linalg.inv(intrinsics[:3, :3]))
    print("Offsets ", np.min(poses[:,:3,3], axis=0), np.max(poses[:, :3,3], axis=0))
