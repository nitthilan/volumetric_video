...
def read_depth_values(flags):
    ...
    for idx, file in enumerate(sorted(file_lst[:-1])[::10]):
        ...
        print(pts_cld.shape)
        acc_pts_cld = np.append(acc_pts_cld, pts_cld[::20,:], axis=0)
    ...
    print("Pts cld ", acc_pts_cld.shape, np.min(acc_pts_cld, axis=0),
        np.max(acc_pts_cld, axis=0))
    ...
    pcd.points = o3d.utility.Vector3dVector(acc_pts_cld)
