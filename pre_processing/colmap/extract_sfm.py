...
def run_matcher(basefolder):
    # image_folder = os.path.join(basefolder, "images")
    # colmap_folder = os.path.join(basefolder, "colmap")
    # os.makedirs(colmap_folder, exist_ok = True)
    # database_path = os.path.join(colmap_folder, "database.db")
    # sparse_path = os.path.join(colmap_folder, "sparse")
    # os.makedirs(sparse_path, exist_ok = True)

    # run_colmap.run_sift_matching_gpu(image_folder, database_path)
    # run_colmap.run_sfm(image_folder, database_path, sparse_path)

    colmap_cameras, colmap_images, colmap_points3D = \
        read_model(os.path.join(basefolder, "0"), '.bin')
    camera_dict_file = os.path.join(basefolder, 'cameras.json')

    camera_dict = parse_camera_dict(colmap_cameras, colmap_images)
    with open(camera_dict_file, 'w') as fp:
        json.dump(camera_dict, fp, indent=2, sort_keys=True)

    normalize_cam_dict(camera_dict_file, os.path.join(basefolder, 'cameras_normalized.json'))


    # all_tracks, all_points, view_keypoints = parse_tracks(colmap_images, colmap_points3D)
    # all_points = np.array(all_points)
    # xyz_file = os.path.join(colmap_folder, 'xyz_points.txt')
    # np.savetxt(xyz_file, all_points, header='# format: x, y, z, reproj_err, track_len, color(RGB)', fmt='%.6f')

    # mesh = trimesh.Trimesh(vertices=all_points[:, :3].astype(np.float32), 
    #                        vertex_colors=all_points[:, -3:].astype(np.uint8))
    # mesh.export(os.path.join(colmap_folder, 'mesh_points.ply'))

    # with open(os.path.join(colmap_folder, 'track.txt'), 'w') as fp:
    #     json.dump(all_tracks, fp)

    # with open(os.path.join(colmap_folder, 'keypoint.txt'), 'w') as fp:
    #     json.dump(view_keypoints, fp)

    return
if __name__ == '__main__':
    basefolder = sys.argv[1]
    run_matcher(basefolder)
