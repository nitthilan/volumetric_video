...
def get_pose_json(base_dir):

    output = {}
    file_list = os.listdir(os.path.join(base_dir, 'pose'))
    for file in file_list:
        pose_file = os.path.join(base_dir, 'pose', file)
        intrinsics_file = os.path.join(base_dir, 'intrinsics', file)
        intrinsic = np.loadtxt(intrinsics_file)
        pose = np.loadtxt(pose_file)
        # print(pose, intrinsic)
        output[file[:-4]+".png"] = {
            'K':intrinsic, 
            'W2C':np.linalg.inv(pose.reshape((4, 4))),
            'C2W':pose,
            'img_size': np.array([1008, 548])}
    return output

if __name__ == '__main__':
    import os

    playground_video_path = "../data/output_6th_nov_2021/Playground_video/cameras_normalized.json"
    playground_train_cpu_path = "../data/output_6th_nov_2021/Playground_train_cpu/cameras_normalized.json"
    playground_dataset_path = "../data/output_6th_nov_2021/Playground_dataset/"

    base_dir = './'

    sphere_radius = 1.
    playground_video = json.load(open(playground_video_path))
    playground_train_cpu = json.load(open(playground_train_cpu_path))
    playground_dataset = get_pose_json(playground_dataset_path)

    print("Output ", playground_train_cpu)
    camera_size = 0.1
    colored_camera_dicts = [
                            ([0, 1, 0], playground_video),
                            ([0, 0, 1], playground_train_cpu),
                            ([1, 0, 0], playground_dataset)
                            ]
