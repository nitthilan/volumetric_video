...
def run_sift_matching_cpu(img_dir, db_file, remove_exist=False):
...
def run_sift_matching_gpu(img_dir, db_file, remove_exist=False):
    print('Running sift matching...')
    ...
    # feature extraction
    # if there's no attached display, cannot use feature extractor with GPU
    cmd = 'xvfb-run -s "+extension GLX -screen 0 1024x768x24" colmap feature_extractor \
            --database_path {} \
            --image_path {} \
            --ImageReader.single_camera 1 \
            --ImageReader.camera_model SIMPLE_RADIAL \
    ...
    # feature matching
    cmd = 'xvfb-run -s "+extension GLX -screen 0 1024x768x24" colmap \
        exhaustive_matcher --database_path {} \
                 --SiftMatching.guided_matching 1 \
                 --SiftMatching.use_gpu 1 \
                 --SiftMatching.max_num_matches 65536 \
                 --SiftMatching.max_error 3 \
                 --SiftExtraction.gpu_index {}'.format(db_file, gpu_index)
...
if __name__ == '__main__':
    img_dir = './data/pixologic/andrey_malygin_1/images/'
    out_dir = './data/pixologic/andrey_malygin_1/colmap_mv/'
    # img_dir = '../data/pixologic/eric_keller_3/images/'
    # out_dir = '../data/pixologic/eric_keller_3/colmap_mv/'
    run_mvs = True
